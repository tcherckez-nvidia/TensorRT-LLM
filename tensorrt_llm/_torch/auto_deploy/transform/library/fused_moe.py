...
def _stack_nvfp4_trtllm_gen_moe_weights(
    ...
    def _extract_internal_routing_args(
        ...
        def _match_noaux_getitem(node: Node, expected_index: int) -> Optional[Node]:
            ...
            if not isinstance(source_node, Node) or not is_op(source_node, torch.ops.trtllm.noaux_tc_op):
                return None
            return source_node
    ...
    for node in matched_nodes:
        ...
        if internal_routing_args is None:
            raise RuntimeError(
                "fuse_nvfp4_moe with backend='trtllm_gen' and use_internal_routing=True "
                "requires selected_experts/routing_weights to come from trtllm.noaux_tc_op."
            )
